  ...
  <div class="department-container">
    ...
    <div class="bg-white rounded-lg shadow-md p-4 mb-4 overflow-hidden">
      ...
      <div v-if="!data.loading && data.tableData.length === 0" class="py-8 text-center">
        ...
        <p class="mt-2 text-gray-500">暂无部门数据</p>
        <el-button 
          type="primary" 
          @click="handleAdd" 
...
const save = async () => {
  ...
  try {
    ...
    const api = isUpdate ? '/departments/update' : '/departments/add';
    ...
    if (res.code === '200') {
      ElMessage({
        type: 'success',
        message: isUpdate ? '部门更新成功' : '部门添加成功',
        duration: 2000
      });
...
const del = async (id) => {
  try {
    ...
    const res = await request.delete(`/departments/delete/${id}`);
...
const delBatch = async () => {
  ...
  try {
    await ElMessageBox.confirm(
      `确定要删除选中的 ${data.ids.length} 个部门吗？删除后数据无法恢复。`, 
      '批量删除确认', 
      { 
        type: 'warning',
    ...
    const res = await request.delete("/departments/delete/batch", { data: data.ids });
    
    if (res.code === '200') {
      ElMessage.success(`成功删除 ${data.ids.length} 个部门`);
